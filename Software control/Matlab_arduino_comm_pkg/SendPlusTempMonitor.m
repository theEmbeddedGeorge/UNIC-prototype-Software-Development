...
Serial_port = 'COM4'; %specify serial port
Serial_port2 = 'COM3'; %specify serial port
...
arduino2 = serial(Serial_port2);  %setup serial port communication
set(arduino,'BaudRate',BaudRate);  %setup serial port baudRate
set(arduino2,'BaudRate',BaudRate);  %setup serial port baudRate
...
fopen(arduino2); %'open up' the serial port object 
...
for i=1:rows
  for j=1:columns
    ...
    fwrite(arduino2, Volt_send(i,j), 'uint16');
  end
end
...
% counter = 0;
% pause(5);
% while (temp_sig == 0) %loop forever  %t <= Size) %loop 32*40 times. Each time receive one data   
...
% 
% if temp_sig == 1
%     warning('Unusual temperature detected!');
% end
%% 
