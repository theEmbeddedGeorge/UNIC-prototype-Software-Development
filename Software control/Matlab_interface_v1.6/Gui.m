...
brd7_x1 = [-47.42865289,-32.85869648,-10.20455265,-21.25457577,12.67611726,-5.324864507,-53.95900524,6.591988045,-0.345289417,-62.1094067];

brd1_x2 = [7200.133039,7178.297102,7528.040615,7340.724726,7235.852148,7275.19719,7278.337196,7580.332786,7327.352041,7329.905586];
...
brd4_x2 = [7107.492375,7159.855137,7162.698201,7197.508089,7136.11715,6876.18521,6871.149296,6887.479562,6869.848174,6869.804803];
...
for i=1:rows
    for j=1:columns
        if double(Volt_data(i,j)) > pos_Vmax
            ...
            msg = 'Find values exceed positive max voltage! Rectify unusual value to Vmax.';
            set(handles.Display_text,'String',msg);
        elseif double(Volt_data(i,j))< neg_Vmax
            Volt_data(i,j) = neg_Vmax;
            msg = 'Find values smaller than negative max voltage! Rectify unusual value to -Vmax.';
            set(handles.Display_text,'String',msg);
        end
    end
end          
...
fprintf(arduino1_sent,'S');
fprintf(arduino2_sent,'S');
fprintf(arduino3_sent,'S');
fprintf(arduino4_sent,'S');
fprintf(arduino5_sent,'S');
fprintf(arduino6_sent,'S');
fprintf(arduino7_sent,'S');
...
%     end 
    ...
    %% Read board 5 current vals
       ...
       if ad_val7 == 1
           ...
           temp7_double = str2double(temp7(2:end));
           switch temp7(1)
               case 'A'
...
if ~strcmp(arduino4_sent.port,'NULL')
    ...
    while (button_state == get(hObject,'Max'))
        ...
        button_state = get(hObject,'Value');
        %Current_Monitor_Callback(handles.Current_Monitor, eventdata, handles);
        pause(Sweep_interval_time);
    end 
