...
int trigger = 1; //The trigger pin is D1
  ...
  if (sBuffer.length () > 0)
  {
    ...
    switch (sBuffer [0])            // look at first character in string
    {
      ...
      case 'Q':        // I added this just to test on/off of dac every 30ms
        ...
        break;
      case 'S':
        AutomatedMode(sBuffer.substring (1));
        break;
      default:
        break; 
    }
...
void TestDacCommand (String PsCommand)
{
  ...
  {
    //Add this for emergency stop to work while sweeping
    if (Serial.available () > 0)
    {
      String temp_Buffer;
      temp_Buffer = Serial.readStringUntil('\n');
      if (temp_Buffer[0] == 'Z' || temp_Buffer[0] == 'z') 
        break;
    }
...
void AutomatedMode(String sequence_length) {
  int  Seq_len = sequence_length.toInt();
  long Store_seq [10][Seq_len];
  
  for (int j = 0; j< 10; j++)  //intialize Sequence Matrix
    for (int i = 0; i< Seq_len; i++)  
       Store_seq[j][i] = 0;

  Serial.println('X');  //tells Matlab Arduinos are ready to take data
  int data_counter = 0;
  
  while (Serial.available () > 0 && data_counter < Seq_len*10)  //Store Sequence data
  {
    String Temp_str = Serial.readStringUntil('\n');
    
    int tens;
    tens = int(data_counter/10);  //calculate the tens digit (Channel Number)
    tens = tens-(int(tens/10)*10);
    
    Store_seq[tens][data_counter%10] = Temp_str.toFloat(); //Create the Matrix
    data_counter ++;
  }
  
  int Seq_loc = 0;  //record location in the output Sequence
  
  while (data_counter == Seq_len)  //Automated mode while loop
  {
    if (Serial.available()>0) 
    {
      String Eval_str = Serial.readStringUntil('\n');
      if (Eval_str[0] == 'z' || Eval_str[0] == 'Z')   //Exit Automated mode if Matlab ask to do so 
          break; 
    }
     
    //Current distribution to channels
    if (digitalRead(trigger) == HIGH)  //if detects MRI trigger signal comes in
    {
      for (int i = 0; i < OUTPUTPINCOUNT; i++)
      {
        digitalWrite (MaOutputPins [i], LOW);  // set chip enable on
        SPI.transfer(0xA0);                    // set to 24 bit +/- 5v bipolar range
        SPI.transfer(Store_seq[i][Seq_loc]>>8);    // only upper 8 bits
        SPI.transfer(Store_seq[i][Seq_loc]);       // always leave lower 8 bits the same
        digitalWrite (MaOutputPins [i], HIGH); // set chip enable off
      }
    }
    Seq_loc = (Seq_loc+1)%Seq_len;
  }
}
