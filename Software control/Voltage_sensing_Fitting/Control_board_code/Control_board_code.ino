...
void setup()
{
  ...
  delay( 300 );
  Serial.println("************************");
  Serial.println("For Auto-fitting mode please type: Q ");
  Serial.println("Type 'Z' to reset Channel values or stop Auto-fitting mode");
  Serial.println("Type 'x' + 'number' to output all channel at the same current. eg. 'x45800' ");
  Serial.println("Type 'Channel letter' + 'number' to output signle channel at specific current. eg. 'A45800' ");
  Serial.println("************************");
}
  ...
  if (sBuffer.length () > 0)
  {
    ...
    switch (sBuffer [0])            // look at first character in string
    {
      ...
      case 'J':
        ...
        break;
      case 'X':
        for (int temp_dac = 1; temp_dac < 11; temp_dac++)
         {
           dactype = temp_dac;
           SaveDacCommand (sBuffer.substring (1));  
         }
         break;
      case 'Q':        
...
void TestDacCommand (String PsCommand)
{
  ...
  for (i = 0; i < OUTPUTPINCOUNT; i++)
  {
    digitalWrite (MaOutputPins [i], LOW);  // set chip enable on
    SPI.transfer(0xA0);                    // set to 24 bit +/- 5v bipolar range
    SPI.transfer(vlt>>8);                     // only upper 8 bits
  ...
  while (it < 8)
  {
    if (stop_sig == 1)
      {
        Serial.println("Fitting mode stops.");
        break;
      }
    
    if (it % 2 == 0)
      offset = -(0x03E8);   // current value that is changed on each pass
    else
      offset = 0x03E8;    // make current negative
