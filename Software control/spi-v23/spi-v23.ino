...
int OUTPUTPINCOUNT = 4;     // number of dacs or opa channels in use max will be 10 for new pc board
    ...
    {             
      ...
      case 'Q':        // I added this just to test on/off of dac every 30ms
        ...
        for (i = 0; i < 100; i++)      // run 100 times
          {
          ProcessDacCommand (sBuffer.substring (1));  // set dacs to a value
          delay( 30 );                                // time ms of dacs on
          dacZeroTest();                              // set dacs to zero
          delay( 30 );
          }
        break;
...
// command takes 16.88us for any one dac chip enable down and backup
{
  ...
  for (i = 0; i < OUTPUTPINCOUNT; i++)
  {
    ...
    Serial.print("data written to dac on channel=");
    Serial.print( i );
...
void SaveDacCommand (String PsCommand)
{
  ...
  Serial.print(" store dac channel=");
...
void HuiHelp()
{
 Serial.println("Help for Output Amp testing Ver. 9-21-2015");
