...
void TestDacCommand (String PsCommand)
{
  ...
  { 
   for (s = 0; s < 7; s++)
   {
    ...
    for (i = 0; i < OUTPUTPINCOUNT; i++)
    {
      Serial.print(" testing_dac_channel=");
      Serial.print(i);
      Serial.print("  High_Hex_Volts=");
      Serial.print(vlt,HEX); 
      digitalWrite (MaOutputPins [i], LOW);  // set chip enable on
      SPI.transfer(0xA0);                    // set to 24 bit +/- 5v bipolar range
      ...
      Serial.print(temperatureC,0);              // show calculated temperature
      Serial.println("C");     
    }
    for (i =0; i < 60; i++)               // this sets how long the channel is on for
    {
